...
//                    Log.i(TAG, "onPreviewFrame: ft costTime = " + (System.currentTimeMillis() - ftStartTime) + "ms");
                ...
                int processCode = ftEngine.process(nv21, previewSize.width, previewSize.height, FaceEngine.CP_PAF_NV21, faceInfoList, FaceEngine.ASF_MASK_DETECT | FaceEngine.ASF_FACE_SHELTER);
            ...
            if (faceShelterInfoList.size() == maskInfoList.size() && faceShelterInfoList.size() == faceInfoList.size()) {
                for (int i = 0; i < faceInfoList.size(); i++) {
                    facePreviewInfoList.add(new FacePreviewInfo(faceInfoList.get(i),
                            faceShelterInfoList.get(i).getFaceShelter(),
