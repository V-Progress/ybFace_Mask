...
public class FaceManager {
    ...
    public void init(Context context) {
        ...
        synchronized (this) {
            if (compareEngin == null && context != null) {
                compareEngin = new FaceEngine();
                int compareInitCode = compareEngin.init(context, DetectMode.ASF_DETECT_MODE_IMAGE, DetectFaceOrientPriority.ASF_OP_ALL_OUT, 16, 1, FaceEngine.ASF_FACE_RECOGNITION | FaceEngine.ASF_FACE_DETECT);

                Log.e(TAG, "init: 初始化对比引擎结果：" + compareInitCode);
            }
        }
        reloadRegisterList();
    }
    ...
    public boolean registerBgr24(@NonNull String fileName, byte[] bgr24, int width, int height) {
        synchronized (this) {
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                code = compareEngin.extractFaceFeature(bgr24, width, height, FaceEngine.CP_PAF_BGR24,
...
//                code = compareEngin.extractFaceFeature(bgr24, width, height, FaceEngine.CP_PAF_BGR24, faceInfo, faceFeature);
    ...
    public CompareResult compare(final FaceFeature faceFeature) {
        if (compareEngin == null || faceFeature == null || faceRegisterInfos == null) {
            return null;
        }
