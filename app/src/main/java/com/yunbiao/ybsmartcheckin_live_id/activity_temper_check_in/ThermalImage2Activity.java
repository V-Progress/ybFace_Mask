...
public class ThermalImage2Activity extends BaseThermal2Activity implements ThermalViewInterface {
    ...
    @Override
    protected void setMaskDetectEnable(boolean enable) {
        faceView.setMaskDetectEnable(enable);
    }
...
//            tvThermalTemper.setText(getResources().getString(R.string.main_thermal_temp) + temper + "℃");
    ...
    public void clearAllUI() {
        ...
        if (llMaskTip.isShown()) {
            llMaskTip.setVisibility(View.GONE);
        }
        setBigHeadDottedLine();
    }
    ...
    public boolean isMaskTipsShown() {
        return llMaskTip.isShown();
    }

    @Override
    public void showMaskTip(String tip) {
        tvMaskTip.setText(tip);
        if (!llMaskTip.isShown()) {
            llMaskTip.setVisibility(View.VISIBLE);
        }
    }
